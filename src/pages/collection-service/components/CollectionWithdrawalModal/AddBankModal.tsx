...
import { addCollectionPayoutBank } from '@/services/collection-service/api';
...
const AddBankModal: React.FC<IProps> = (props) => {
    ...
    const [banks, setBanks] = useState<ObjectType[]>([]);
    ...
    const handleCreateBank: any = async (values: any) => {
        ...
        console.log(data);
        const resp: any = await addCollectionPayoutBank(data);
        if (resp.success) {
    ...
    useEffect(() => {
        ...
        (async function () {
            ...
            if (resp.success) {
                const mappedBanks = resp?.data?.map((item: any) => {
                    return {
        ...
        >
            ...
            >
                ...
                <FormSelect
                    ...
                    fieldProps={{
                        onChange(value) {
                            const bankShortName = banks.find(propertyEqual('value', value))?.label || '';
                            setBankName(bankShortName);
                            formRef?.current?.setFieldsValue({ bankName });
                        }
                    }}
                />
